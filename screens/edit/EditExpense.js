...
import { editExpenseItem, editItem } from '../../src/Store/actions/AuthAction';
...
export default function EditItem({route, navigation }) {
    ...
    const doEditItem=()=>{
        dispatch(editExpenseItem(setloading,item.id,getyear,getmonth,getdate,title,amount,comment))
    }
    return (
        <ScrollView style={{flex:1}} >
            <View >
                <View style={{ flexDirection: 'row', alignItems: 'space-between', paddingHorizontal: 25, justifyContent: 'space-between', paddingTop: 20 }}>
                    ...
                    <Text style={{ fontSize: 22, color: 'black' }}>Edit Expense</Text>
            ...
            <View style={{ flex: 1 }}>
                            ...
                            <View style={{ marginTop: 40, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 20 }}>
                              {loading ? <LoadingButton/> :<AppButton name='Edit Expense' onPress={doEditItem}  />}
